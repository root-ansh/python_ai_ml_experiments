...
def clone_repo(repo):
    ...
    branches_output = get_command_output('cd {} && git branch -r'.format(repo_name))
    branches = []

    for branch_line in branches_output.split('\n'):
        # Extract branch name from stuff like "  origin/feat/foo -> origin/feat/foo\n")
        branch_name = branch_line.split(" ")[-1].split("/", 1)[-1]

        if len(branch_name) > 0 and branch_name not in branches:
            branches.append(branch_name)

    print('Found {} branches'.format(len(branches)))
    default_branch_name = branches[0]
